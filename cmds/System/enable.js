		...
		if (args[0]){ // Check that user has provided arguments
			if (client.commands.has(args[0])) { // Check if provided argument corresponds to an existing command
				...
				if (cmdExists){
					...
					if (!isEnabled){
						const m = await message.channel.send(`Enabling: ${args[0]}`);
						await cmdExists.update({
							enabled: true
						});
						await m.edit(`Successfully enabled: ${args[0]}`);
						log.info(`${message.member.displayName} (${message.author.username}#${message.author.discriminator}) has enabled ${args[0]} on ${message.guild.name}.`);
					} else {
						//If it is enabled return.
						message.reply(`Command "${args[0]}" is already enabled in ${message.guild.name}.`);
						return;
					}
				} else {
