...
exports.func = (bot) => {
	return new Promise(async (resolve, reject) => {
		try {
				...
				return new Promise(async (resolve, reject) => {
					try {
						...
						if (watcher.globalEnable) { // Load the watcher if it is globally enabled
							bot.log.verbose(`Loading Watcher: ${props.data.name}. 👌`);
							...
							bot.log.verbose(`Skipped loading ${props.data.name} as it is disabled. ❌`);
			...
			bot.log.info(chalk.green(`Loaded ${loadedList.length} watcher(s) (${loadedList.join(', ')}) and skipped ${skippedList.length} (${skippedList.join(', ')}).`));
			resolve(bot.watchers); // Return the watchers collection
		} catch (err) {
			bot.log.error(`Error in loadWatchers: ${err}`);
