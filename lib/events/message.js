exports.func = (bot, msg) => {
    ...
    try {
        ...
        bot.log.error(`Something went wrong when handling a message: ${err}`);
    }
}

// Update User
async function updateUser (bot, msg){
	//Check if user exists in db
	const userExists = await bot.ProfilesModel.findOne({
		where: {
			guildId: msg.guild.id,
	...
	if (userExists){
		...
		await bot.ProfilesModel.create({
			guildId: msg.guild.id,
			username: msg.author.username,
